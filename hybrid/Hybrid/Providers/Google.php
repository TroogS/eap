...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
	function getUserProfile($target = "me")
	{
		...
		if (strpos($this->scope, '/auth/plus.profile.emails.read') !== false) {
			$verified = $this->api->api( "https://www.googleapis.com/plus/v1/people/{$target}" );

			if ( ! isset( $verified->id ) || isset( $verified->error ) )
				$verified = new stdClass();
		} else {
			$verified = $this->api->api( "https://www.googleapis.com/plus/v1/people/{$target}/openIdConnect" );

			if ( ! isset( $verified->sub ) || isset( $verified->error ) )
				$verified = new stdClass();
		}

		$response = $this->api->api( "https://www.googleapis.com/plus/v1/people/{$target}" );
